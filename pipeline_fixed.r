...
extract_mff_lookup <- function(df, sheet_name, study_value, cpms_id) {
  ...
  # Visit_Name from the MFF row already holds the human-readable visit label
  # (e.g. "Screening", "Day 30"). Rename to Visit_Label to match the corrected schema.
  # MFF rows are visit-level cost summaries, not tied to a single activity, so Activity_Name is NA.
  tdf <- tdf %>%
...
expand_to_visit_rows_legacy <- function(df, study_value, cpms_id, study_arm_value, visit_label_lookup = NULL) {
  # visit_label_lookup: optional named character vector mapping Visit_Number -> Visit_Label
  # (e.g. c("VISIT - 001" = "Screening", "VISIT - 002" = "Day 30"))
  # When NULL, Visit_Label is filled with the standardised Visit_Number as a fallback.
  ...
  for (i in seq_len(nrow(df))) {
    ...
    for (j in seq_along(visit_cols)) {
      ...
      # Visit_Label: resolved from lookup if provided, otherwise fall back to Visit_Number.
      # This is the human-readable visit name (e.g. "Screening", "Day 30").
      visit_label <- if (!is.null(visit_label_lookup) && visit_number %in% names(visit_label_lookup)) {
        visit_label_lookup[[visit_number]]
      } else {
        visit_number   # safe fallback — unambiguous, never an activity name
      }
...
build_ua_ssp_lookup_from_sheet <- function(df, study_value, cpms_id, visit_label_lookup = NULL) {
  # visit_label_lookup: named character vector of Visit_Number -> Visit_Label,
  # built from the MFF row and passed through so Visit_Label is populated correctly
  # in expand_to_visit_rows_legacy rather than falling back to Visit_Number.
...
run_stage_a <- function(input_file, db_dir = NULL) {
  ...
  for (sheet_name in sheet_names) {
    ...
    # Build visit_label_lookup from the MFF rows for this sheet:
    # named vector of Visit_Number -> Visit_Label (e.g. "VISIT - 001" -> "Screening").
    # Passed into build_ua_ssp_lookup_from_sheet so that UA/SC/SSP rows carry the
    # correct Visit_Label and are no longer reliant on the overloaded Visit_Name column.
